...
bool GameScene::SwitchScene(const char* lpszmapname)
{
	...
	if(!bSuc)
	{
		AfxGetHge()->System_Log("�����ͼ[%s]ʧ��", lpszmapname);
		return false;
	}
	else
	{
		AfxGetHge()->System_Log("�����ͼ[%s]�ɹ�", lpszmapname);
	}
...
bool GameScene::SwitchScene(DWORD _id)
{
	...
	if(NULL == pMapInfo)
	{
		AfxGetHge()->System_Log("�����ڵĵ�ͼID[%d]",
			_id);
		return false;
	}
	else
	{
		AfxGetHge()->System_Log("�����ͼID[%d]",
			_id);
		//return true;
	}
...
void GameScene::ProcKeyCmd()
{
	...
	*/
		...
		bCtrlState)
	{
		...
		{
			GameInfoBoardDlg::GetInstance()->InsertBoardMsg(ARGB_WHITE, "��������������ʾ�ű��ɹ�");
		}
	}
		...
		bCtrlState)
	{
		...
		{
			GameInfoBoardDlg::GetInstance()->InsertBoardMsg(ARGB_WHITE, "������������Ի��ű��ɹ�");
		}
	}
		...
		bCtrlState)
	{
		...
		sprintf(szMsg, "��ã���Ϸ���氡����123 XXX fe9s ���ǵ�X�����档������");
		...
		AfxGetHge()->Input_KeyUp(HGEK_S))
	{
		...
		if(pTheGame->Config_JpgSnap())
		{
			...
			{
				DeleteFile(szBuf);
				sprintf(szBuf, "�ѱ���: snapshot\\%04d%02d%02d%02d%02d.jpg",
		...
		{
			sprintf(szBuf, "�ѱ���: snapshot\\%04d%02d%02d%02d%02d.bmp",
				st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
			//m_pMainOpt->GetChatDlg()->GetHistoryDlg()->InsertChatMessage(szBuf, ARGB_RED);
			GameInfoBoardDlg::GetInstance()->InsertBoardMsg(ARGB_RED, szBuf);
		}
	...
	if(strlen(szText) > 0)
	{
		if(szText[0] == '@')
		{
			...
			if(0 == strcmp(szCmd, "move"))
			{
				...
				{
					m_pMainOpt->GetChatDlg()->GetHistoryDlg()->InsertChatMessage("�﷨:@move x,y", ARGB_RED);
				}
			}
			...
			else if(0 == strcmp(szCmd, "rloadri"))
			{
				...
				m_pMainOpt->GetChatDlg()->GetHistoryDlg()->InsertChatMessage("�ض����������Ϣ", ARGB_RED);
			}
			else if(0 == strcmp(szCmd, "rloadmri"))
			{
				GameInfoManager::GetInstance()->ReloadMagicRenderInfo();
				m_pMainOpt->GetChatDlg()->GetHistoryDlg()->InsertChatMessage("�ض�ħ��������Ϣ", ARGB_RED);
			}
			...
			{
				m_pMainOpt->GetChatDlg()->GetHistoryDlg()->InsertChatMessage("[����]��Чָ��", ARGB_RED);
			}
		}
		else if(szText[0] == '!')
		{
		...
		else if(szText[0] == '~')
		{
			if(strlen(szText) > 56)
			{
				m_pMainOpt->GetChatDlg()->GetHistoryDlg()->InsertChatMessage("[����]�������ݹ���", ARGB_RED);
			}
			else
			{
				bRet = true;
			...
			else
			{
				...
				if(2 == sscanf(szText, "/%s %s", szSenderName, szMsg))
				{
					...
					if(0 == nSendLen ||
						nSendLen > 19)
					{
						m_pMainOpt->GetChatDlg()->GetHistoryDlg()->InsertChatMessage("[����]��Ч�������", ARGB_RED);
					}
					else if(0 == nMsgLen)
					{
						m_pMainOpt->GetChatDlg()->GetHistoryDlg()->InsertChatMessage("[����]�����뷢����Ϣ", ARGB_RED);
					}
					else
					{
...
// 				}
			...
			{
				AfxGetHge()->System_Log("��ȡ����[%d]������Ϣʧ��", not.uMonsID);
				delete pNewNPC;
			}
		}
		else
		{
			AfxGetHge()->System_Log("HandleID��ͻ���޷�ע����д���");
			delete pNewNPC;
		}
	}
	else
...
void GameScene::OnAssistWndClose() {
	GetMainOpt()->GetChatDlg()->GetHistoryDlg()->InsertChatMessage("[��ʾ]���ѹرո������ڣ������� @assist ���ٴδ򿪴���", ARGB_RED);
}
...
{	
	...
	/*if(AfxGetHge()->Input_KeyUp(HGEK_ENTER))
	{
		m_pMsgBoard->InsertBoardMsg("���Ե�%d����Ϣ", ++nLine);
	}*/
}
#endif